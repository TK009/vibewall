...
class PypiDownloadsCheck(BaseCheck):
    name = "pypi_downloads"
    abbrev = " DL"
    ...
    async def run(self, target: str, context: CheckContext) -> CheckResult:
        ...
        url = f"https://pypistats.org/api/packages/{encoded}/recent"
        try:
            ...
            logger.warning("pypi_downloads_timeout", package=target)
            return CheckResult.err("downloads request timed out")
        except aiohttp.ClientError as e:
            logger.warning("pypi_downloads_error", package=target, error=str(e))
